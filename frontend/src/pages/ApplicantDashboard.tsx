...
import { getSession, isApplicant } from "@/lib/authStore";
...
const ApplicantDashboard = () => {
  ...
  const { data: apps = [], isLoading, refetch } = useGetApplicationsQuery(
    ...
    { skip: !session || !isApplicant(session) }
  );
  const [deleteApp] = useDeleteApplicationMutation();

  useEffect(() => {
    if (!session || !isApplicant(session)) { navigate("/login"); return; }
  }, []);
  ...
  const approvedStatuses = ["approved", "sanctioned"];
  const rejectedStatuses = ["rejected"];
  const approved = apps.filter(a => approvedStatuses.includes(a.status)).length;
  const rejected = apps.filter(a => rejectedStatuses.includes(a.status)).length;
  const active = apps.filter(a => !approvedStatuses.includes(a.status) && !rejectedStatuses.includes(a.status)).length;

  const filteredApps = useMemo(() => {
    let result = [...apps];
    if (statFilter === "active") result = result.filter(a => !["approved", "sanctioned", "rejected"].includes(a.status));
    else if (statFilter === "approved") result = result.filter(a => ["approved", "sanctioned"].includes(a.status));
  ...
  if (isLoading) {
    return (
      <AppLayout title="Venture Debt Fund" subtitle="Applicant Portal" noPadding>
        ...
        <main className="p-6 space-y-6">
          <div className="flex items-start justify-between">
            <div className="gov-section-header">
              ...
              <p className="text-sm text-muted-foreground mt-1">Track and manage your Venture Debt Fund financing applications.</p>
            </div>
                    ...
                    <TableRow key={app.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          {(app.workflowStep === "prelim_review" || app.workflowStep === "prelim_submitted" || app.workflowStep === "prelim_revision") && (app.status === "pending_review" || app.status === "submitted" || app.status === "reverted") && (
