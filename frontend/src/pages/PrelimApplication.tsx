...
const prelimSchema = z.object({
  aifName: z.string().max(250, "Max 250 characters").optional().default(""),
  businessCompanyName: z.string().min(1, "Business / Company Name is required").max(250, "Max 250 characters"),
  cin: z.string()
    .regex(/^[LU][0-9]{5}[A-Z]{2}[0-9]{4}[A-Z]{3}[0-9]{6}$/, "Invalid CIN format (e.g., U12345AB2019PTC123456)")
  ...
  ...docSlotFields,
  docPitchDeck: z.string().min(1, "Pitch Deck is required"),  // override — required
});

type PrelimFormValues = z.infer<typeof prelimSchema>;

const defaultValues: Partial<PrelimFormValues> = {
  aifName: "", businessCompanyName: "", cin: "", businessModel: "", otherParticulars: "", otherParticularsAmt: "",
...
const PrelimApplication = () => {
  ...
  const isSidbi = session !== null && (session.roles?.length ?? 0) > 0 && !session.roles?.includes("admin");
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="bg-card border border-border">
              <div className="gov-section-header bg-muted px-6 py-3 border-b border-border">
                <h2 className="font-bold text-foreground text-sm uppercase tracking-widest">Business Description</h2>
              </div>
              <div className="px-6 py-6 space-y-5">
                <FormField control={form.control} name="businessCompanyName" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="font-semibold text-xs uppercase tracking-wide">Business / Company Name {!isReadOnly && <span className="text-destructive">*</span>}</FormLabel>
                    {isReadOnly
                ...
                <FormField control={form.control} name="cin" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="font-semibold text-xs uppercase tracking-wide">Corporate Identification Number (CIN) {!isReadOnly && <span className="text-destructive">*</span>}</FormLabel>
                    {isReadOnly
                      ? <p className="text-sm text-foreground py-2 border-b border-border font-mono">{field.value || "—"}</p>
                      : <>
                          <FormControl><Input placeholder="e.g. U12345AB2019PTC123456" className="font-mono uppercase" {...field} onChange={e => field.onChange(e.target.value.toUpperCase())} /></FormControl>
                          <p className="text-xs text-muted-foreground mt-1">21-character CIN as per MCA format (L/U + 5 digits + 2 letters state code + 4-digit year + 3 letters + 6 digits).</p>
                        </>
                    }
                    <FormMessage />
                  </FormItem>
                ...
                <FormField control={form.control} name="aifName" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="font-semibold text-xs uppercase tracking-wide">Name of AIF / Investment Manager (IM)</FormLabel>
                    {isReadOnly
                      ? <p className="text-sm text-foreground py-2 border-b border-border">{field.value || "—"}</p>
              ...
              <div className="px-6 py-6 space-y-4">
                {prelimDocumentSlots.map((slot) => (
                  <FormField key={slot.key} control={form.control} name={slot.key as DocSlotKey} render={({ field }) => (
                    <FormItem>
                      <FormLabel className="font-semibold text-xs uppercase tracking-wide">
                        {slot.label}
                        {slot.key === "docPitchDeck" && !isReadOnly && <span className="text-destructive"> *</span>}
                      </FormLabel>
                      {isReadOnly ? (
