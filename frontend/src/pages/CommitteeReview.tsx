...
const CommitteeReview = () => {
  ...
  const isConvenor = session?.sidbiRole === "convenor";
  const isCommitteeMember = session?.sidbiRole === "committee_member"
  ...
  const handleConsent = async (meeting: (typeof meetings)[0], value: "approve" | "reject") => {
    ...
    try {
      ...
      toast({
        ...
        description: value === "approve" ? "Your consent has been recorded for this meeting." : "Your response has been recorded.",
      });
    } catch (e) {
      toast({
  ...
  const handleSendForConsent = async (meeting: (typeof meetings)[0]) => {
    try {
      await updateStatus({ id: meeting.id, status: "in_progress" }).unwrap();
      simulateEmail(
        meeting.selectedMembers.map((m) => m.email),
        `${isICVD ? "IC-VD" : "CCIC-CGM"} Meeting — Consent Required`,
        `${meeting.subject} has been sent for your consent. Please review and submit your response.`
      );
      toast({
        title: "Sent for Consent",
        description: `Meeting sent to committee members. They can now submit their consent.`,
      });
      navigate("/sidbi/dashboard");
    } catch (e) {
      toast({
        title: "Failed to send for consent",
        description: e instanceof Error ? e.message : "Please try again.",
        variant: "destructive",
      });
    }
  ...
  return (
    <AppLayout title="Venture Debt Fund" subtitle="Committee Review" backTo="/sidbi/dashboard" backLabel="Back to Dashboard"
        ...
        {meetings.length === 0 ? (
          <div className="bg-card border border-border p-12 text-center">
            <p className="text-muted-foreground">
              {isCommitteeMember
                ? `No ${isICVD ? "IC-VD" : "CCIC-CGM"} meetings sent for your consent yet.`
                : `No scheduled ${isICVD ? "IC-VD" : "CCIC-CGM"} meetings pending review.`}
            </p>
          </div>
        ) : (
                  ...
                  <div className="flex justify-end gap-3">
                    {isConvenor ? (
                      <Button
                        ...
                        onClick={() => handleSendForConsent(meeting)}
                      >
                        Send to Committee for Consent
                      </Button>
                    ) : isCommitteeMember ? (() => {
                      const myVote = meeting.votes?.find((v) => v.memberId === session?.email);
                      const hasVoted = !!myVote;
                      const isVoting = votingMeetingId === meeting.id;
                      return (
                          ...
                          {hasVoted ? (
                            ...
                            <>
                              <Button
                                ...
                                className="font-bold uppercase tracking-wider text-xs"
                                disabled={isVoting}
                                onClick={() => handleConsent(meeting, "reject")}
                              >
                                No
                              </Button>
                              <Button
                                className="font-bold uppercase tracking-wider text-xs"
                                disabled={isVoting}
                                onClick={() => handleConsent(meeting, "approve")}
                              >
                                Yes
                              </Button>
                            </>
