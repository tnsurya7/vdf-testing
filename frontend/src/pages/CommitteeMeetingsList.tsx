...
const CommitteeMeetingsList = () => {
  ...
  const { data: allMeetings = [] } = useGetMeetingsQuery({ type: type as MeetingType });
  ...
  const meetings = isCommitteeMember
    ? allMeetings.filter((m) => m.status === "in_progress" || m.status === "completed")
    : allMeetings;
  ...
  return (
    <AppLayout title="Venture Debt Fund" subtitle="Committee Meetings">
            ...
            {meetings.length === 0 ? (
              <div className="p-8 text-center">
                <p className="text-sm text-muted-foreground">
                  {isCommitteeMember ? "No meetings sent for your consent yet." : `No ${title} meetings scheduled yet.`}
                </p>
              </div>
            ) : (
