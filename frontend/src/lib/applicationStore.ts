...
import type { PrelimApplicationData, DetailedApplicationData, CommitteeNoteData } from "@/lib/types";
...
export const workflowTransitions: Record<WorkflowStep, WorkflowAction[]> = {
  ...
  ccic_referred:            [],
  ccic_maker_sanction:      ["ccic_maker_sanction"],
  // Legacy CCIC
...
const actionTransitions: Partial<Record<WorkflowAction, WorkflowStep>> = {
  ...
  recommend_icvd:             "icvd_convenor_scheduling",
  ...
  ccic_committee_refer:       "ccic_maker_sanction",
  ccic_maker_sanction:        "sanctioned",
  // Legacy CCIC
...
export interface Application {
  ...
  // Data
  prelimData: PrelimApplicationData | null;
  detailedData: DetailedApplicationData | null;
  icvdNote?: CommitteeNoteData | null;
  ccicNote?: CommitteeNoteData | null;
  // Meeting references
...
export function createPrelimApplication(email: string, prelimData: PrelimApplicationData): Application {
  const apps = getApplications();
...
export function submitDetailedApplication(appId: string, detailedData: DetailedApplicationData): void {
  const apps = getApplications();
...
export function applyWorkflowAction(
  ...
  if (action === "reject_prelim" || action === "reject_final" || action === "reject_sanction") {
    ...
    apps[idx].status = "submitted";
    apps[idx].stage = "ccic";
  } else if (action === "ccic_maker_sanction") {
    apps[idx].status = "sanctioned";
    apps[idx].stage = "post_sanction";
  }
...
export function updatePrelimData(id: string, prelimData: PrelimApplicationData): void {
  const apps = getApplications();
  const idx = apps.findIndex((a) => a.id === id);
  if (idx !== -1) {
